...
class HarnessConsoleUXStateDriver:
    ...
    def append_info_line(self, text: str, color: str | None = None) -> None:
        ...
        effective_color = color or get_mode_color(
            self._state.mode_text, self._mode_colors
        )

        # Add separator when transitioning from streaming text
        prefix = ""
        if self._last_entry_type in (
            OutputEntryType.STREAMING_TEXT,
            OutputEntryType.STREAM_FOOTER,
        ):
    ...
    def write_text(self, text: str, color: str | None = None) -> None:
        ...
        if (
            ...
            and self._current_streaming_entry_index
            == len(self._state.output_entries) - 1
        ):
